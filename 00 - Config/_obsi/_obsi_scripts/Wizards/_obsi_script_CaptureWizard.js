...
// several notes (an NPC's factions) get _obsi_script_MultiSelectPrompt afterwards.
//
...
module.exports = async (tp, domain) => {
    ...
    for (const field of spec.fields) {
        ...
        if (field.input === "noteMulti") {
            ...
            if (files.length) multiFields.push({ field, files });
            else formFields.push({ key: field.key, label: field.label, placeholder: field.hint });
            continue;
        }
    ...
    for (const { field, files } of multiFields) {
        const picked = await tp.user._obsi_script_MultiSelectPrompt({
            question: `${field.label} — ${spec.type.toLowerCase()} "${answers.name}"`,
            options: files.map(f => ({ label: f.basename, sublabel: f.path, value: f })),
            filterPlaceholder: `Filter ${field.label.toLowerCase()}…`,
        });
        // Cancelled → leave the line on its {hint} rather than dropping the capture.
        answers[field.key] = (picked || []).map(f => f.basename);
    }
