...
| `LocationHierarchy` | The nesting rules shared by both location wizards and the `location_move` view: `tierOf(frontmatter, categories)`, `allowedChildTypes(categories, parentTier)`, `bucketFor(picked)`, `folderUnderParent(parentFolder, picked)`, `folderAtCampaignRoot(campaignRoot, picked)`. Called as `_obsi_script_LocationHierarchy()` — it returns the API object. **`bucketFor` is the only place a category's folder is decided** — both folder functions go through it, because they once disagreed and put the same City in `Cities/` with a parent and `City/` without one. `location_move` reads `allowedChildTypes` from the other end (which PARENTS fit this type, not which types fit this parent) off the same comparison, so a move can only land a note where the wizards would have created it |
...
| `LoreWizard` | name, lore type (8), then THREE picker buttons — related lore, related players, related NPCs | `icon`, `iconColor`, `lore_type`, `relations`, `locations` (**the world alone** — not asked for). All three pickers feed the ONE `relations` list, deduplicated and minus this note itself: `relations` is the field every scoped *Lores* callout filters on, so an NPC or PC picked here is what makes the entry show up on their page | 
